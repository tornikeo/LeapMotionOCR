...
def load_training_data(path="training_data") -> tuple[list, list]:
    ...
    labels = (
        pd.Series(data).astype(str).str.extract(r".+stroke_(\d)_.+.csv").astype(int)[0]
    )
...
class KMeansClassifier(BaseEstimator, TransformerMixin, AccuracyScorerMixin):
    ...
    def fit(
        ...
        centers = np.random.uniform(
            low=X.min(), high=X.max(), size=(self.n_cluster, n_features)
        )
        it = 0
        while True:
            if it > self.max_iter:
...
class SVC(BaseEstimator, AccuracyScorerMixin):
    ...
    def fit(self, X, y):
        ...
        solution = np.linalg.solve(
            P, q, overwrite_a=True, overwrite_b=True, check_finite=False
        )
...
class KNN(BaseEstimator, AccuracyScorerMixin):
    ...
    def predict(self, x: np.ndarray):
        ...
        distances = np.linalg.norm(
            x_train[:, np.newaxis, :] - x[np.newaxis, :, :], axis=2
        )

        # Find the indices of the k-nearest neighbors for each test point
        nearest_neighbors_indices = np.argpartition(
            distances, self.n_neighbors, axis=0
        )[: self.n_neighbors, :]
        ...
        predictions = np.apply_along_axis(
            lambda x: np.bincount(x).argmax(), axis=0, arr=nearest_neighbors_labels
        )
        return predictions


def cross_val_score(
    clf: BaseEstimator, x_train: np.ndarray, y_train: np.ndarray, cv: int
) -> np.ndarray:
...
class PCA(BaseEstimator, TransformerMixin):
    ...
    def fit(self, x: np.ndarray, y: np.ndarray = None):
        assert (
            np.isclose(x.mean(0), 0).all() and np.isclose(x.std(0), 1).all()
...
def evaluate(
    ...
    """
    cv_score = cross_val_score(clf, x_train_scale, y_train, cv=4)
